...
try:
    from documents.applet import DocumentApplet
except ImportError:
    print("Document applet not found. Skipping.")
    DocumentApplet = None
...
class App(ctk.CTk):
    ...
    def toggle_sidebar(self, show=True):
        """Toggle sidebar visibility for split view mode"""
        if show:
            self.sidebar_frame.grid(row=1, column=0, sticky="nsew")
        else:
    ...
    def show_documents_home(self):
        ...
        self.current_view = "documents"
        self.toggle_sidebar(True)  # Always show sidebar when going home
        self.hide_all_views()
    ...
    def set_theme_colors(self, colors):
        ...
        if self.current_view == "todo":
            ...
            self.show_documents_home()
        elif self.current_view == "documents_applet":
            self.open_documents()
    ...
    def open_documents(self):
        """Load document applet into main content area with caching"""
        self.current_view = "documents_applet"
        self.hide_all_views()
        
        # Check if we have a cached view
        if "documents_applet" in self.cached_views and self.cached_views["documents_applet"].winfo_exists():
            self.cached_views["documents_applet"].pack(fill="both", expand=True)
            self.current_applet = self.cached_applets.get("documents_applet")
            return
        
        # Create new view
        container = ctk.CTkFrame(self.main_content_frame, fg_color="transparent")
        container.pack(fill="both", expand=True)
        self.cached_views["documents_applet"] = container
        
        if DocumentApplet:
            self.current_applet = DocumentApplet(container)
            self.cached_applets["documents_applet"] = self.current_applet
        else:
